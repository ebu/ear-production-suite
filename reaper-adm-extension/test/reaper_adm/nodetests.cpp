...
TEST_CASE("With a node creator and a simple object") {
    ...
    SECTION("when a track node is created") {
        auto trackNode = creator.createObjectTrackNode(std::vector<adm::ElementConstVariant>(1, simpleObject.audioObject), nullptr);
...
//        auto trackVal{ 2 };
        ...
        SECTION("When an automation node is created") {
            ...
            auto node = creator.createAutomationNode(ADMChannel{simpleObject.audioChannelFormat, simpleObject.audioPackFormat, simpleObject.audioTrackUid}, takeElement);
            SECTION("It is not null") {
                REQUIRE(node);
            }
        }

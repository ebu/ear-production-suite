...
class ADMChannel {
public:
    ADMChannel(std::shared_ptr<adm::AudioChannelFormat const> channelFormat,
    ...
    friend inline bool operator== (ADMChannel const& lhs, ADMChannel const & rhs) {
        return (lhs.trackUid() == rhs.trackUid() && lhs.channelFormat() == rhs.channelFormat() && lhs.packFormat() == rhs.packFormat());
    }

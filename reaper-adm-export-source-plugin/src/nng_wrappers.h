...
public:
    ...
    int getSeqReadPos() {
        return seqReadPos / dataSize;
    }

    bool atSeqReadEnd() {
        return (!buf) || (seqReadPos >= getDataCount());
    }
    ...
    bool getDataAt(T* val, int startingDataPoint, int numOfDataPoints = 1) {
        ...
        if (startingDataPoint < 0 || (startingDataPoint + numOfDataPoints) > getDataCount() || !buf) return false;
...
class CommandCommon {
public:
    enum Command { GetConfig, StartRender, StopRender };
};
...
public:
    ...
    CommandReceiver(callback commandHandler) : SocketBaseForListeners(), commandHandler{ commandHandler } {}
    ~CommandReceiver() {}

    enum Command { GetConfig, StartRender, StopRender };

    void sendInfo(uint8_t channels, uint32_t sampleRate, uint16_t admTypeDefinition, uint16_t admPackFormatId, uint16_t admChannelFormatId) {
        nng_msg* msg;
...
private:
    ...
    int openSpecifics() override {
        ...
        if (resOpen == 0) {
            auto resSize = nng_setopt_size(socket, NNG_OPT_RECVMAXSZ, 0); //Enforce no limit on message size (for now)
            assert(resSize == 0);
            auto resBuf = nng_setopt_int(socket, NNG_OPT_RECVBUF, 1000); //Max messages sitting in buffer
            assert(resBuf == 0);
            auto resAioAlloc = nng_aio_alloc(&aio, CommandReceiver::internalAioCallback, this);
    ...
    int listenSpecifics() override {
        ...
        int res;
        while (portSuffix < NNG_PORT_END && (res = nng_listen(socket, fullUrl.c_str(), NULL, 0)) == NNG_EADDRINUSE) {
            portSuffix++;
...
public:
    ...
    int sendBlock(std::shared_ptr<NngMsg> msg, int timeOut = 0) {
        ...
        return nng_send(socket, msg->getBufferPointer(), msg->getSize(), (timeOut == 0) ? NNG_FLAG_NONBLOCK : 0);
    }
...
private:
    int openSpecifics() override {
        ...
        if (resOpen == 0) {
            ...
            auto resBuf = nng_setopt_int(socket, NNG_OPT_RECVBUF, 1000); //Max messages sitting in buffer
            assert(resBuf == 0);
            resBuf = nng_setopt_int(socket, NNG_OPT_SENDBUF, 1000); //Max messages sitting in buffer
            assert(resBuf == 0);
        }
...
public:
    ...
    std::shared_ptr<TypedNngMsg<float>> receiveBlock() {
        ...
        nng_setopt_ms(socket, NNG_OPT_SENDTIMEO, 100);
        msg->setResult(nng_recv(socket, msg->getBufferPointerPointer(), msg->getSizePointer(), NNG_FLAG_ALLOC));//NNG_FLAG_NONBLOCK | NNG_FLAG_ALLOC));
        return std::move(msg);
    }
